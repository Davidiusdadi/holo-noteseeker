...
pub fn create_note(note: Note) -> ExternResult<Record> {
  ...
  let record = get(note_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest(String::from("Could not find the newly created Note"))))?;
...
pub fn update_note(input: UpdateNoteInput) -> ExternResult<Record> {
  let updated_note_hash = update_entry(input.original_action_hash, &input.updated_note)?;

  let record = get(updated_note_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest(String::from("Could not find the newly updated Note"))))?;
